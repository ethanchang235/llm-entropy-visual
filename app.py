...
MODEL_NAME = "gpt2" # Or "distilgpt2" or any other default you prefer
MODEL_DISPLAY_NAME = "GPT-2 (Small)" # User-friendly name

DEFAULT_TOP_K_PREDICTIONS = 15
MAX_TOP_K_PREDICTIONS = 50
DEFAULT_TEMPERATURE = 1.0
MIN_TEMPERATURE = 0.01
MAX_TEMPERATURE = 2.0 # Adjusted max a bit for more noticeable effect range

# --- Application State ---
model_state: Dict[str, Any] = {
    "model_name": MODEL_NAME, # Fixed model name
    ...
    "loaded": False,
    "loading_error": None,
    "is_loading": False, # To track the initial load
}
...
def get_next_token_predictions(text: str, top_k: int, temperature: float) -> Dict[str, Any]:
    ...
    logging.debug(f"Predicting for: '{text[:50]}...', Top K: {top_k}, Temp: {temperature:.2f}")
...
@app.route('/')
def index_page():
    # Pass model display name to the template
    return render_template('index.html', model_display_name=model_state['model_display_name'])

@app.route('/status', methods=['GET'])
def get_status():
    if model_state["is_loading"]: # This refers to the initial load
        return jsonify({
            "status": "loading_model",
            "model_name": model_state["model_display_name"], # Show display name
...
def get_entropy_api():
    ...
    try:
        ...
        logging.info(f"API Request: Text (len {len(text)}), TopK: {top_k}, Temp: {temperature:.2f}, Model: {model_state['model_name']}")
        predictions = get_next_token_predictions(text, top_k=top_k, temperature=temperature)
        end_time = time.time()

        if "error_message" in predictions:
             logging.warning(f"Prediction error: {predictions['error_message']}")
        else:
            logging.info(f"API Response: Entropy={predictions['entropy']:.4f}, TopTok='{predictions['top_k_tokens'][0]}', Time={end_time - start_time:.3f}s")
...
if __name__ == '__main__':
    ...
    initial_load_success = load_model_and_tokenizer() # Loads the single configured model
